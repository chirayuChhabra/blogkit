...
To embed a simulation, place your JavaScript file in the `sims/` directory and use the the standard image syntax tag:

```html
![Drag the planets to change their orbits.](../sims/gravity.js)
```
...
> [!note]
> There is also the the standard image syntax tag, which behaves exactly the same but hides the "Click to interact" overlay, intended for passive motion demos.
...
> 
> Mr Markdown will automatically discover your `.config.json` file as long as it sits right next to your javascript file and shares the same base name.
